...
run_case() {
	...
	if [[ $encoding == "POSE" ]]; then
		...
		./runlim -r "$TO" -s "$MO" "$PYTHON_BIN" -u pipeline_evalmaxsat.py "$dataset" POSE 2>&1 | tee "$log_file"
	else
		group="DSE_${card}"
		log_file="results/pipeline/$group/${dataset}.log"
		mkdir -p "$(dirname "$log_file")"
		./runlim -r "$TO" -s "$MO" "$PYTHON_BIN" -u pipeline_evalmaxsat.py "$dataset" DSE "$card" 2>&1 | tee "$log_file"
	fi
}
