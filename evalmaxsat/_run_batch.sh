...
if [[ $# -lt 2 ]]; then
	echo "Usage: $0 <processing|no_preprocessing> <POSE|DSE|CARD> [exact_encoding] [amo_encoding] [datasets...]" >&2
	exit 2
fi
...
case "$mode" in
	processing)
		script_name="evalmaxsat.py"
		;;
	no_preprocessing)
		script_name="evalmaxsat_no_processing.py"
		;;
	*)
...
run_case() {
	...
	log_file="results/${mode}/pipeline/${group}/${dataset}.log"
