...
def build_cpsat_cp_model(var, ctr_file):
    model = cp_model.CpModel()

    f = {}
    for i, vals in var.items():
        f[i] = model.NewIntVarFromDomain(
            cp_model.Domain.FromValues(sorted(vals)),
            f"f_{i}",
        )
    ...
    for i, vals in var.items():
        for value in vals:
            b = model.NewBoolVar(f"b_{i}_{value}")
            indicator[(i, value)] = b
            model.Add(f[i] == value).OnlyEnforceIf(b)
            model.Add(f[i] != value).OnlyEnforceIf(b.Not())
            model.AddImplication(b, y[value])
        model.AddExactlyOne(indicator[(i, value)] for value in vals)

    all_values = label_set
    ...
    with open(ctr_file) as file:
        for line in file:
            ...
            diff = model.NewIntVar(0, max_diff, f"diff_{i}_{j}")
            model.AddAbsEquality(diff, f[i] - f[j])

            if ">" in parts:
                model.Add(diff > distance)
            elif "=" in parts:
                model.Add(diff == distance)
...
def extract_solution(solver, f):
    return {i: solver.Value(var) for i, var in f.items()}
...
class IncumbentPrinter(cp_model.CpSolverSolutionCallback):
    def __init__(self, f, start_time):
        cp_model.CpSolverSolutionCallback.__init__(self)
        self._f = f
        self._start_time = start_time

    def on_solution_callback(self):
        labels = {self.Value(var) for var in self._f.values()}
        print("Number of labels used:", len(labels))
...
def run(preprocess):
    ...
    print(f"Building CP-SAT CP-style model for {len(var)} variables...")
    model, f, y = build_cpsat_cp_model(var, files["ctr"])
    print("Number of frequency variables:", len(f))
    ...
    print("Solving with OR-Tools CP-SAT CP-style model...")
    callback = IncumbentPrinter(f, start_time)
    ...
    assignment = extract_solution(solver, f)
    print("Solution:")
