...
def main():
    ...
    process = psutil.Process(os.getpid())
    print(f"Memory used: {process.memory_info().rss / 1024**2:.2f} MB")
    solver.delete() # delete old solver to free memory

    while True:
        print("\nTrying to find better solution with at most", num_lables - 1, "labels...")
        solver = Solver(name='cadical195')
        build_constraints(solver, var, var_map, files["ctr"])
        lable_var_map = create_label_var_map(domain[0], solver.nof_vars() + 1)
        build_label_constraints(solver, var_map, lable_var_map)
        rhs = add_limit_label_constraints(solver, lable_var_map, num_lables - 1)
        assignment = solve_and_print(solver, var_map, rhs, num_lables, 'first')
        if assignment is None:
            print("Optimal solution found with", num_lables, "labels.")
            print(f"Total time: {time.perf_counter() - start_time:.2f} seconds")
            break
        else:
            num_lables = len(set(assignment.values()))
