...
def add_limit_label_constraints(solver, lits, num_labels):
    tot = ITotalizer(lits=lits, ubound=num_labels, top_id=solver.nof_vars())
...
def main():
    ...
    rhs = add_limit_label_constraints(solver, label_vars, num_labels)
    ...
    while True:
        ...
        solver.add_clause([-rhs[best_K - 1]])
        if solver.solve():
            model = solver.get_model()
            assignment = {}
            for (i, v), varnum in var_map.items():
