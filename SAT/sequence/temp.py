...
def print_domains(var,var_temp):
    print("Variable domains:")
    for i in sorted(var.keys()):
        print(f"Variable {i}: {var_temp[i]}")
        print(f"Removed  {i}: {var[i]}")
        print("----------------------")
...
def verify_solution_simple(assignment, var, var_file,ctr_file):
    ...
    with open(ctr_file) as f:
        for line in f:
            ...
            if i not in assignment or j not in assignment:
                print("Assignment missing variable")
                return False
            vi = assignment[i]
            vj = assignment[j]
            if(vi not in var[i]) or (vj not in var[j]):
                print("Assignment not in domain")
                return False
            if '>' in parts:
                distance = int(parts[4])
                if abs(vi - vj) <= distance:
                    print(f"Distance constraint violated: {i}({vi}) - {j}({vj}) = {abs(vi - vj)} <= {distance}")
                    print(f"Domains {i}: ", var[i])
                    print(f"Domains {j}: ", var[j])
                    return False
            elif '=' in parts:
                value = int(parts[4])
                if abs(vi - vj) != value:
                    print(f"Distance equality constraint violated: {i}({vi}) - {j}({vj}) = {abs(vi - vj)} != {value}")
                    print(f"Domains {i}: ", var[i])
                    print(f"Domains {j}: ", var[j])
                    return False
...
def main():
    ...
    if assignment is None:
        return
    if verify_solution_simple(assignment, var, files["var"], files["ctr"]):
