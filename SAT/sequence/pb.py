...
def get_file_names(dataset_folder):
    ...
    if base.lower().startswith("graph") or base.lower().startswith("tud"):
        return {
...
def pb(solver, label_var_map, UB, type):
    ...
    elif type == "pb_binary_merge":
        config.set_PB_Encoder(pblib.PB_BINARY_MERGE)
    else:
        print(type)
        raise ValueError("Unknown PB encoding type(pblib.PB_BDD, pblib.PB_BINARY_MERGE).")
...
def card(solver, label_var_map, UB, type):
    first_free_var = solver.nof_vars() 
    x_vars = []
    for i in range(UB):
        first_free_var += 1
    ...
    if type == "card_seq":
        encoding = 1
    elif type == "card_cardnet":
        encoding = 3
    else:
        raise ValueError(
            "Unknown CARD encoding type "
            "(card_seq, card_cardnet)."
        )

    top_id = solver.nof_vars() + 1

    formular = CardEnc.atmost(
        lits=label_vars,
        bound=UB,
        top_id=top_id,
        encoding=encoding
...
def add_limit_label_constraints(solver, label_var_map, UB, type):
    if "pb" in type:
        return pb(solver, label_var_map, UB, type)
    elif "card" in type:
        return card(solver, label_var_map, UB, type)
    else:
        print(type)
        raise ValueError("Unknown encoding type (pb_bdd, pb_binary_merge, card_seq, card_cardnet).")
...
def main():
    ...
    type_encoding = {"pb_bdd", "pb_binary_merge", "card_seq", "card_cardnet"}
    if len(sys.argv) < 3:
        print("Use: python main.py <dataset_folder> <type pb encoding> <solver>")
        return
    if sys.argv[2] not in type_encoding:
        print(f"Unknown encoding type {type_encoding}")
        return
    if sys.argv[3] not in solvers:
        print(f"Unknown sovler {solvers}")
        return
    ...
    x_vars = add_limit_label_constraints(solver, lable_var_map,num_lables, config)
