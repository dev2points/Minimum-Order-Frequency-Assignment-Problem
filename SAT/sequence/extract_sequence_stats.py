...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Rebuild sequence SAT formulas from old logs and extract variable/clause counts."
    )
    parser.add_argument(
        "inputs",
        nargs="+",
...
def normalize_script_name(raw_name: str, log_path: Path) -> str:
    script_name = os.path.basename(raw_name)
    if script_name in {"main.py", "main_no_preprocessing.py"}:
        return script_name
    if script_name == "no_preprocessing.py":
        return "main_no_preprocessing.py"
    if "no_pre_processing" in str(log_path) or "no_preprocessing" in str(log_path):
        return "main_no_preprocessing.py"
    return "main.py"


def extract_trying_bounds(lines: List[str]) -> List[int]:
    return [int(match.group(1)) for line in lines for match in [TRYING_RE.search(line)] if match]
...
def extract_target_bound(lines: List[str]) -> tuple[int, str, str]:
    ...
    trying_bounds = extract_trying_bounds(lines)
    last_trying = trying_bounds[-1] if trying_bounds else None
    ...
    if any("Solve first problem:" in line for line in lines) and any("Cannot find solution." in line for line in lines):
        return 0, "base_unsat", "no_trying_line"
    if any("Cannot find solution!" in line for line in lines):
        return 0, "preprocess_unsat", "no_formula"
    raise ValueError("Cannot infer target bound from log.")


def infer_build_bound(strategy: str, trying_bounds: List[int]) -> int:
    if not trying_bounds:
        return 0
    if strategy == "tot":
        return trying_bounds[0] + 1
    return trying_bounds[0]
...
def parse_log_config(log_path: Path) -> LogConfig:
    ...
    script_name = normalize_script_name(argv[2], log_path)
    dataset = argv[3]
    strategy = argv[4]
    sat_type = argv[5]
    solver_name = argv[6]
    trying_bounds = extract_trying_bounds(lines)
    build_bound = infer_build_bound(strategy, trying_bounds)
    ...
    return LogConfig(
        ...
        trying_bounds=trying_bounds,
        no_formula=target_source == "no_formula",
    )
...
def incremental_unit_clause_count(config: LogConfig) -> int:
    if config.sat_type != "incremental":
        return 0
    if config.strategy == "tot":
        return len(config.trying_bounds)
    return max(len(config.trying_bounds) - 1, 0)
...
def build_stats(config: LogConfig) -> StatsRow:
    ...
    silent_call(module.build_constraints, solver, var, var_map, top_id, files["ctr"])
    base_vars = solver.nof_vars()
    base_clauses = solver.nof_clauses()

    label_var_map = silent_call(module.create_label_var_map, domain[0], solver.nof_vars() + 1)
    after_label_vars = base_vars
    after_label_clauses = base_clauses
    after_bound_vars = base_vars
    after_bound_clauses = base_clauses

    if config.build_bound > 0:
        ...
        silent_call(module.add_limit_label_constraints, solver, label_var_map, config.build_bound, config.strategy)
        after_bound_vars = solver.nof_vars()
        after_bound_clauses = solver.nof_clauses()

        for _ in range(incremental_unit_clause_count(config)):
            solver.add_clause([1])
    ...
    return StatsRow(
        ...
        solver_name=config.solver_name,
        preprocessed=config.preprocessed,
        build_bound=config.build_bound,
        target_bound=config.target_bound,
        status=config.status,
        target_source=config.target_source,
        num_tries=len(config.trying_bounds),
        base_vars=base_vars,
        base_clauses=base_clauses,
        label_vars=after_label_vars - base_vars,
        label_clauses=after_label_clauses - base_clauses,
        bound_vars=after_bound_vars - after_label_vars,
        bound_clauses=after_bound_clauses - after_label_clauses,
...
def print_row(row: StatsRow) -> None:
    print(
        f"{Path(row.log_path).name}: build={row.build_bound}, target={row.target_bound}, "
        f"vars={row.total_vars}, clauses={row.total_clauses}, "
        f"base=({row.base_vars},{row.base_clauses}), "
        f"label=({row.label_vars},{row.label_clauses}), "
        f"bound=({row.bound_vars},{row.bound_clauses}), "
        f"tries={row.num_tries}"
    )
