...
def add_limit_label_constraints(solver, lits, K):
    ...
    n = len(lits)
    lits = [0] + lits  # 1-based indexing
    top = solver.nof_vars()

    # r[i][j] = r(i,j), i=1..n, j=1..K
    ...
    # (1) ¬x_i ∨ r(i,1)
    for i in range(1, n):
        solver.add_clause([-lits[i], r[i][1]])
    # (2) ¬r(i-1,j) ∨ r(i,j)
    for i in range(2, n):
    ...
    for i in range(1, K + 1):
        solver.add_clause([lits[i], -r[i][i]])
    ...
    for i in range(2, n):
        for j in range(1, min(i-1, K)+1):
            solver.add_clause([lits[i], r[i-1][j], -r[i][j]])

    # (8) ¬x_i ∨ ¬r(i-1,K)
    for i in range(K+1, n+1):
        solver.add_clause([-lits[i], -r[i-1][K]])
    # rhs[j-1] ⇔ sum(x) ≤ j
    rhs = r[n - 1]
...
def main():
    ...
    while num_labels > 1:
        ...
        # build_constraints(solver, var, var_map, last_var_num, files["ctr"])
        # build_label_constraints(solver, var_map, lable_var_map)
        # # Thêm ràng buộc giới hạn số nhãn sử dụng
        # rhs = add_limit_label_constraints(solver, lable_var_map, num_labels - 1)
        solver.add_clause([-rhs[num_labels]])
        assignment = solve_and_print(solver, var_map,)
        if assignment is None:
