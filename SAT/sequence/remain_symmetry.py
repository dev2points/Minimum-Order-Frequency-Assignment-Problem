    ...
    with open(ctr_file) as f:
        for line in f:
            ...
            if '=' in parts:
                ...
                for jv in vals_v:
                    solver.add_clause([-var_map[(v, jv)]] + [var_map[(u, iu)] for iu in vals_u if abs(iu - jv) == distance]) #(5)
            elif '>' in parts:
                for iu in vals_u:
                ...
                for jv in vals_v:
                    if (jv - distance <= vals_u[0]):
                        for iu in vals_u:
                            if iu - jv > distance:
                               solver.add_clause([-var_map[(v, jv)], order_var_map[(u, iu)]]) #(9)
                               break
                        ...
                        for t in vals_u:
                            if t >= T:
                                solver.add_clause([-var_map[(v, jv)], -order_var_map[(u, t)]]) #(10)
                                break
                    else : # (11)
